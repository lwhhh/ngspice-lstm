...
double read_step(int num){
    FILE *fp;
    int flag_r;
    double action_step;
    while(1){
    fp=fopen("a_read.txt","r");
    while(fp==NULL){
    fp=fopen("a_read.txt","r");  
    }
    fscanf(fp,"%d %lg",&flag_r,&action_step);
    fclose(fp);
    if(flag_r==num){    
       fp=fopen("a_read.txt","w");
       while(fp==NULL){
       fp=fopen("a_read.txt","w");  
       }
       fprintf(fp,"%d %lg",num+1,action_step);
       fclose(fp);
       break;
    }
    }
...
//void write_state(int num,int iter_num,double tol,int converged,double diff ,int final){
//    FILE *fp; 
//    fp=fopen("a_write.txt","w");
//    while(fp==NULL){
//    fp=fopen("a_write.txt","w"); 
//    }
//    fprintf(fp,"%d %d %g %d %g %d",num+1,iter_num,tol,converged,diff,final);
//    fclose(fp);
//  }
void write_state(int num, int iter_num, double tol, int converged, double diff, int final, double ckttol, double cktdiff) {
    FILE* fp;
    fp = fopen("a_write.txt", "w");
    while (fp == NULL) {
        fp = fopen("a_write.txt", "w");
    }
    fprintf(fp, "%d %d %lg %d %lg %d %lg %lg", num + 1, iter_num, tol, converged, diff, final,  ckttol,  cktdiff);
    fclose(fp);
}
...
double difference;
double truncError;
int state(CKTcircuit *ckt, double *tol,double *diff);
...
initalFile() {
    ...
    int readFlag = 2;
    int writeFlag = 1;
    readfile = fopen("C:\\Users\\KM\\Desktop\\a_read.txt", "w");
    fprintf(readfile, "%d\n", readFlag);
    writefile = fopen("C:\\Users\\KM\\Desktop\\a_write.txt", "w");
    fprintf(writefile, "%d\n", writeFlag);
    fclose(readfile);
    fclose(writefile);
    printf("initialize completely!\n");
}
void writePrint(int NRs, double trunc, int accFlag, int size, CKTcircuit* ckt) {
    printf("********************** write data by simulator ***********************\n");
    printf("NRs:%d, truncError:%le, accFlag:%d\n", NRs, trunc, accFlag);
    for (int i = 0; i < size; i++) {
        printf("%dth rhs:%le,", i, *(ckt->CKTrhs + i));
    }
    printf("\n");
}
void writeData(int Flag, int stop, int NRs, double trunc, int accFlag, int xsize, CKTcircuit* ckt, int NRnum) {
    int error;
    double deltademo;
    ckt->use_Qdiff_flag = 0;//add dyc just get the Qstate ,do not use the predict Q which will be use in the truely CKTtrunc process
    error = CKTtrunc(ckt, &deltademo);
    ckt->use_Qdiff_flag = 0;
    FILE* writeFile;
    int writeFlag;
    while (1) {
        writeFile = fopen("C:\\Users\\KM\\Desktop\\a_write.txt", "r+");
        if (writeFile != NULL) {
            fscanf(writeFile, "%d\n", &writeFlag);
            if (writeFlag == 2 * Flag - 3 ) {
                printf("*************** write data by simulator! *****************\n");
                fflush(stdout);
                ...
                fprintf(writeFile,"%d\n", xsize);
                fprintf(writeFile, "%d\n", NRnum);
                fprintf(writeFile,"%d,%.16lf,%d,%.16lf,%.16lf\n", NRs, trunc ,accFlag , ckt->CKTtime,ckt->CKTdelta);
                for (int i = 0; i < xsize - 1; i++) {
                    fprintf(writeFile,"%.16lf,", *(ckt->CKTrhs + i));
                }
                fprintf(writeFile, "%.16lf\n", *(ckt->CKTrhs + xsize - 1));
                fprintf(writeFile, "%d\n", ckt->Qnum);
                if (ckt->Qnum > 0)//add dyc  Stupid compiler (VS, yes) debug does not change Qindex and in fact printf does, fuckit
                {
                    for (int i = 0; i < ckt->Qnum-1; i++)
                    {
                        fprintf(writeFile, "%.16lf,", ckt->QState[i]);
                    }
                    fprintf(writeFile, "%.16lf", ckt->QState[ckt->Qnum-1]);
                }  
                fprintf(writeFile, "\n");
                if (ckt->Qnum > 0)//add dyc  Stupid compiler (VS, yes) debug does not change Qindex and in fact printf does, fuckit
                {
                    for (int i = 0; i < ckt->Qnum - 1; i++)
                    {
                        fprintf(writeFile, "%.16lf,", ckt->deltatimeinQ[i]);
                    }
                    fprintf(writeFile, "%.16lf", ckt->deltatimeinQ[ckt->Qnum - 1]);
                }
                fprintf(writeFile, "\n\n\n\n\n");

                //writePrint(NRs, trunc, accFlag, xsize, ckt);
                //rewind(writeFile);
                //fprintf(writeFile, "%d", writeFlag);
                printf("flag in write:%d\n", writeFlag);
                fclose(writeFile);
...
double predTimeStep;
int usedFlag = 0;
CKTcircuit cktglobal;
double readModel[1000];


//void readPrint(double predTimeStep, int size, CKTcircuit* cktglobal) {
//    printf("****************** read data from model *********************\n");
//    printf("predict timeStep:%le", predTimeStep);
//    for (int i = 0; i < size; i++) {
//        printf("predict %dth solution:%le,", i, *(cktglobal->CKTrhs + i));
//    }
//}

void readData(int Flag, int xsize, CKTcircuit* ckt, int NRnum) {
    FILE* readFile;
    int actionFlag;
    int readsize;
    double aa = 0;
    while (1) {
        readFile = fopen("C:\\Users\\KM\\Desktop\\a_read.txt", "r+");
        if (readFile != NULL) {
            fscanf(readFile, "%d\n", &actionFlag);
            if (actionFlag == 2 * Flag - 3 ) {
                printf("*************** read data from model! *****************\n");
                fflush(stdout);
                ...
                {
                    for (int i = 0; i < ckt->Qnum - 1; i++) {
                        fscanf(readFile, "%lf,", &aa);
                        ckt->pre_timestep_Q[i] = aa;
                    }
                }
                fscanf(readFile, "%lf\n", &ckt->pre_timestep_Q[ckt->Qnum - 1]);
                ckt->pre_timestep = predTimeStep;
                ckt->pre_timestep = ckt->CKTdelta;
                printf("2ckt->pre_time: %.16lf,%.16lf,%.16lf\n", ckt->pre_timestep, ckt->CKTdelta, predTimeStep);
                for (int i = 0; i < xsize - 1; i++) {
                    fscanf(readFile, "%lf,", &aa);
                    readModel[i] = aa;
                }
                ...
                fscanf(readFile, "%lf\n", &ckt->QState[ckt->Qnum-1]); 
                rewind(readFile); 
                fflush(readFile);
                fprintf(readFile, "%d\n", actionFlag);
                printf("flag in read:%d\n", actionFlag);

                fprintf(readFile, "%d\n", readsize);
                fprintf(readFile, "%d\n", usedFlag);
                fprintf(readFile, "%.16lf\n", predTimeStep);
                for (int i = 0; i < xsize-1; i++) {
                    fprintf(readFile, "%.16lf,", readModel[i]);
                }
                fprintf(readFile, "%.16lf\n", readModel[xsize - 1]);
                ...
                fflush(stdout);
                //readPrint(predTimeStep, xsize, cktglobal);
                return;
            }
...
void usePredSolution(double* arr, CKTcircuit* ckt, int size) {
    //ckt->CKTdelta = predTimeStep;
    for (int i = 0; i < size;i++) {
        double aa = 0, bb = 0;
        aa = *(ckt->CKTrhsOld + i);
        bb = *(arr + i);
        *(ckt->CKTrhsOld + i) = *(arr + i);
        *(ckt->CKTrhsOld + i) = *(arr + i);
        aa = *(ckt->CKTrhsOld + i);
        bb = *(arr + i);
    }
...
void usePredQ(CKTcircuit* ckt) {
    for (int i = 0; i < ckt->Qnum; i++) {
        double aa = 0, bb = 0;
        aa = ckt->QState[i];
        ckt->CKTstate0[ckt->Qcap[i]] = aa;
    }
}
       ...
       int restart)   /* forced restart flag */
{
    ...
    int final=0;
    int flag=1; // modify by phj for simple interaction
    int iter_old;
    int iter_numbers;
    double tol;
    double diff;
    ...
    ckt->Qnum = 0;
    ckt->use_Qdiff_flag = 0; //add dyc inital 
    ckt->pre_timestep = 1e-12; //add dyc inital 
    for (int i = 0; i < 2000; i++)//add dyc inital 
    {
        ckt->deltatimeinQ[i] = 0;
    }
    printf("0ckt->pre_timestep :%.16lf\n", ckt->pre_timestep);
#ifdef XSPICE
...
/* gtri - end - wbk - Add Breakpoint stuff */
    ...
    int xsize = 0;
    xsize = SMPmatSize(ckt->CKTmatrix);
    //xsize = 3;
...
#endif
        //state(ckt, &tol,&diff);
        //write_state(2*flag-3,iter_numbers,tol, 1, diff , 1);
        //write_state(2 * flag - 3, iter_numbers, tol, 1, diff, 1, ckt->mintol, ckt->diffmax );
        writeData(flag+1, 1, iter_numbers, 1e+10, 1, xsize, ckt, 1);
        return(OK);
    }
...
/* gtri - end - wbk - Set evt_step */
        ...
        iter_old = ckt->CKTstat->STATnumIter;
        int size = SMPmatSize(ckt->CKTmatrix);
        for (int i = 0; i < size; i++) {
            *(ckt->CKTrhs + i) = *(ckt->CKTrhs + i); //add dyc  zhe shenme wanyier
        }
        if (usedFlag) {
            usePredSolution(readModel, ckt, xsize);
        }
        converged = NIiter(ckt,ckt->CKTtranMaxIter);
        iter_numbers=ckt->CKTstat->STATnumIter-iter_old;
        int boolconverge = 0;
        if (converged != 0)
        {
            boolconverge = 1;
        ...
        writeData(flag, 0, iter_numbers, 1e+10, 1, xsize, ckt, 1);
        readData(flag + 1, xsize, ckt, 1);
        flag++;
#ifdef XSPICE
        if(ckt->evt->counts.num_insts > 0) {
            g_mif_info.circuit.evt_step = ckt->CKTtime;
...
/* gtri - end - wbk - Call all hybrids */
        ...
        if(converged != 0) { // NR not converge
            ...
            writeData(flag, 0, iter_numbers, 1e+10, 1, xsize, ckt, 0);
            readData(flag + 1, xsize, ckt, 0);
#ifdef STEPDEBUG
...
/* gtri - end - wbk - Add Breakpoint stuff */
...
#endif
					firsttime = 0;
                    writeData(flag, 0, iter_numbers, 1e+10, 0, xsize, ckt, 0);
                    readData(flag + 1, xsize, ckt, 0);
#if !defined CLUSTER && !defined SHARED_MODULE
                ...
                {
                    //usePredQ(ckt);
                    ckt->use_Qdiff_flag = 1;
                }
                printf("1ckt->pre_time: %.16lf, %d\n", ckt->pre_timestep, ckt->use_Qdiff_flag);
				error = CKTtrunc(ckt, &newdelta);
                printf("2ckt->pre_time: %.16lf, %d\n", ckt->pre_timestep, ckt->use_Qdiff_flag);
                ckt->use_Qdiff_flag = 0;
				...
				//if (1) {
                    ...
                    writeData(flag, 0, iter_numbers, truncError, 0, xsize, ckt, 0);
                    readData(flag + 1, xsize, ckt, 0);
                    //if (predTimeStep < ckt->CKTdelta)//add dyc gai  
                    //{
                    //    ckt->CKTdelta = predTimeStep;
                    //}
#ifdef NDEV
					...
					ckt->CKTstat->STATrejected++;
					//state(ckt, &tol, &diff);
					//write_state(2*flag-3,iter_numbers,tol, 1, diff , final);
					//write_state(2 * flag - 3, iter_numbers, tol, 1, diff, final, ckt->mintol, ckt->diffmax);
                    writeData(flag, 0, iter_numbers, truncError, 1, xsize, ckt, 0);
                    readData(flag + 1, xsize, ckt, 0);
#else
					redostep = 1;
#endif
...
int
state(CKTcircuit *ckt, double *tol,double *diff)
{
    tolerance=0;
    difference=0;
    int i;
